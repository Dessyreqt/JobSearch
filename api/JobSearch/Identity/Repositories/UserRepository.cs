namespace JobSearch.Identity.Repositories
{
    using System;
    ...
    using JobSearch.Identity.Queries.User;
    ...
    public class
        UserRepository<TUser, TKey, TUserRole, TRoleClaim, TUserClaim, TUserLogin, TRole> : IUserRepository<TUser, TKey, TUserRole, TRoleClaim, TUserClaim, TUserLogin, TRole>
        ...
        where TRole : DapperIdentityRole<TKey, TUserRole, TRoleClaim>
    {
        ...
        public async Task<TUser> GetByEmailAsync(string email)
        {
            try
            {
                var selectFunction = new Func<DbConnection, Task<TUser>>(
                    async x =>
                    {
        ...
        public async Task<TKey> InsertAsync(TUser user, CancellationToken cancellationToken)
        {
            try
            {
                var insertFunction = new Func<DbConnection, Task<TKey>>(
                    async x =>
                    {
                        try
                        {
        ...
        public async Task<bool> InsertClaimsAsync(TKey id, IEnumerable<Claim> claims, CancellationToken cancellationToken)
        {
            try
            {
                var insertFunction = new Func<DbConnection, Task<bool>>(
                    async x =>
                    {
                        try
                        {
            ...
            try
            {
                var insertFunction = new Func<DbConnection, Task<bool>>(
                    async x =>
                    {
                        try
                        {
                            dynamic userLogin = new { UserId = id, loginInfo.LoginProvider, loginInfo.ProviderKey, Name = loginInfo.ProviderDisplayName };
        ...
        public async Task<bool> RemoveAsync(TKey id, CancellationToken cancellationToken)
        {
            try
            {
                var removeFunction = new Func<DbConnection, Task<bool>>(
                    async x =>
                    {
                        try
                        {
        ...
        public async Task<bool> UpdateAsync(TUser user, CancellationToken cancellationToken)
        {
            try
            {
                var updateFunction = new Func<DbConnection, Task<bool>>(
                    async x =>
                    {
                        try
                        {
        ...
        public async Task<IList<Claim>> GetClaimsByUserIdAsync(TKey id)
        {
            try
            {
                var selectFunction = new Func<DbConnection, Task<IList<Claim>>>(
                    async x =>
                    {
                        ...
                        return result?.Select(y => new Claim(y.ClaimType, y.ClaimValue)).ToList();
                    });
        ...
        public async Task<IList<string>> GetRolesByUserIdAsync(TKey id)
        {
            try
            {
                var selectFunction = new Func<DbConnection, Task<IList<string>>>(
                    async x =>
                    {
        ...
        public async Task<IList<UserLoginInfo>> GetUserLoginInfoByIdAsync(TKey id)
        {
            try
            {
                var selectFunction = new Func<DbConnection, Task<IList<UserLoginInfo>>>(
                    async x =>
                    {
                        ...
                        return result?.Select(y => new UserLoginInfo(y.LoginProvider, y.ProviderKey, y.Name)).ToList();
                    });
        ...
        public async Task<IList<TUser>> GetUsersByClaimAsync(Claim claim)
        {
            try
            {
                var selectFunction = new Func<DbConnection, Task<IList<TUser>>>(
                    async x =>
                    {
                        var defaultUser = Activator.CreateInstance<TUser>();
                        var query = _queryFactory.GetQuery<GetUsersByClaimQuery, TUser>(defaultUser);

                        var result = await x.QueryAsync<TUser>(sql: query, param: new { ClaimValue = claim.Value, ClaimType = claim.Type }, transaction: _unitOfWork.Transaction);
            ...
            try
            {
                var selectFunction = new Func<DbConnection, Task<IList<TUser>>>(
                    async x =>
                    {
                        ...
                        var result = await x.QueryAsync<TUser>(sql: query, param: new { RoleName = roleName }, transaction: _unitOfWork.Transaction);
        ...
        public async Task<bool> IsInRoleAsync(TKey id, string roleName)
        {
            try
            {
                var selectFunction = new Func<DbConnection, Task<bool>>(
                    async x =>
                    {
                        var defaultUser = Activator.CreateInstance<TUser>();
                        ...
                        var result = await x.QueryAsync(sql: query, param: new { RoleName = roleName, UserId = id }, transaction: _unitOfWork.Transaction);
                ...
                var removeFunction = new Func<DbConnection, Task<bool>>(
                    async x =>
                    {
                        try
                        {
                            ...
                            foreach (var claim in claims)
                            {
                                resultList.Add(await x.ExecuteAsync(query, new { UserId = id, ClaimValue = claim.Value, ClaimType = claim.Type }, _unitOfWork.Transaction) > 0);
                            }
                ...
                var removeFunction = new Func<DbConnection, Task<bool>>(
                    async x =>
                    {
                        try
                        {
                            ...
                            var result = await x.ExecuteAsync(query, new { UserId = id, RoleName = roleName }, _unitOfWork.Transaction);
                ...
                var removeFunction = new Func<DbConnection, Task<bool>>(
                    async x =>
                    {
                        try
                        {
                            ...
                            var result = await x.ExecuteAsync(query, new { UserId = id, LoginProvider = loginProvider, ProviderKey = providerKey }, _unitOfWork.Transaction);
                    ...
                    async x =>
                    {
                        try
                        {
                            ...
                            var result = await x.ExecuteAsync(
                                query,
                                new { NewClaimType = newClaim.Type, NewClaimValue = newClaim.Value, UserId = id, ClaimType = oldClaim.Type, ClaimValue = oldClaim.Value },
