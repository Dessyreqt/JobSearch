namespace JobSearch.Identity.Repositories
{
    ...
    using JobSearch.Identity.Queries.Role;
        ...
        where TRoleClaim : DapperIdentityRoleClaim<TKey>
    {
        ...
        public RoleRepository(
            ...
            ILogger<RoleRepository<TRole, TKey, TUserRole, TRoleClaim>> log,
        ...
        public async Task<TRole> GetByIdAsync(TKey id)
        {
            try
            {
                var selectFunction = new Func<DbConnection, Task<TRole>>(
                    async x =>
                    {
                        ...
                        return await x.QueryFirstOrDefaultAsync<TRole>(sql: query, param: dynamicParameters, transaction: _unitOfWork.Transaction);
                    });
        ...
        public async Task<IEnumerable<TRoleClaim>> GetClaimsByRole(TRole role, CancellationToken cancellationToken)
        {
            try
            {
                var selectFunction = new Func<DbConnection, Task<IEnumerable<TRoleClaim>>>(
                    async x =>
                    {
                        var dynamicParameters = new DynamicParameters();
                        ...
                        return await x.QueryAsync<TRoleClaim>(sql: query, param: dynamicParameters, transaction: _unitOfWork.Transaction);
                    });
