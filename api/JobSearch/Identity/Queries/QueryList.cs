namespace JobSearch.Identity.Queries
{
    using System;
    ...
    public class QueryList : IQueryList
    {
        ...
        public ConcurrentDictionary<Type, IQuery> RetrieveQueryList()
        {
            if (_dictionary.Count == 0)
            {
                ...
                var exportedTypes = asm.GetTypes().Where(type => type.Namespace != null && type.Namespace.StartsWith($"{nameof(JobSearch)}.{nameof(Identity)}"));

                foreach (var type in exportedTypes)
                {
                    var getConstructorParameters = new Func<Type, List<object>>(
                        x =>
                        {
                            var constructorParameters = type.GetTypeInfo().DeclaredConstructors.FirstOrDefault(y => y.IsPublic).GetParameters();
