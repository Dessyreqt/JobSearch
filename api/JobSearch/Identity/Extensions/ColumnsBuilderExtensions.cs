    ...
    public static class ColumnsBuilderExtensions
    {
            ...
            bool forInsert = true)
        {
            ...
            if (idProperty != null && !ignoreIdProperty)
            {
                ...
                roleProperties = !idPropertyValue.Equals(defaultIdTypeValue) ? entity.GetType().GetPublicPropertiesNames(x => !ignoreProperties.Any(y => x.Name == y)) :
                    forInsert ? entity.GetType().GetPublicPropertiesNames(y => !y.Name.Equals("Id") && !ignoreProperties.Any(x => x == y.Name)) :
                    entity.GetType().GetPublicPropertiesNames(x => !ignoreProperties.Any(y => x.Name == y));
            }
            else
            {
                roleProperties = entity.GetType().GetPublicPropertiesNames(y => !y.Name.Equals("Id") && !ignoreProperties.Any(x => x == y.Name));
            }
