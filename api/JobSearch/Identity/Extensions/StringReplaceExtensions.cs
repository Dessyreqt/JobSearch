namespace JobSearch.Identity.Extensions
{
    ...
    public static class StringReplaceExtensions
    {
        ...
        {
            return query.Replace("%SCHEMA%", schemaName).Replace("%TABLENAME%", tableName).Replace("%COLUMNS%", $"({columns})").Replace("%VALUES%", values);
        }
        ...
        {
            return query.Replace("%SCHEMA%", schemaName).Replace("%TABLENAME%", tableName).Replace("%ID%", idParameter);
        }
        ...
        {
            return query.Replace("%SCHEMA%", schemaName).Replace("%TABLENAME%", tableName).Replace("%SETVALUES%", setValues).Replace("%ID%", idParameter);
        }
            ...
            string[] sqlParameterValues)
        {
            ...
            queryBuilder.Replace("%SCHEMA%", schemaName).Replace("%TABLENAME%", tableName);
        ...
        public static string RemoveSpecialCharacters(this string value) => Regex.Replace(value, @"[^\w\d]", string.Empty);
    }
}